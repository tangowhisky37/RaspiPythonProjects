...
##########################################################################
## Known issues -
## - The log file storing the timestamps needs to be
##   pre-populated with dummy timestamps e.g. "2017-07-13 15:20:22.501775"
##   Make sure you have a few entries in each file incremented by a few
##   seconds each. Without these initial entries the code won't work.
##
## Setup requirements -
...
#########################################################
#AWS Rekognition Code - Face Comparison for Trevor W
#########################################################
def compare_faces(bucket, key, bucket_target, key_target, threshold=80, region="us-west-2"):
	...
	response = rekognition.compare_faces(
        ...
        print "*********************************************************************************"
        print "                                           "
        print "TW: Source Face Confidence in %s " %format(temp2['SourceImageFace']['Confidence'])
        print "                                           "
        print "*********************************************************************************"
        for match in temp2['FaceMatches']:
         print "********************************************************************************"
         print "                                           "
         print "TW: Similarity between compared faces is %s " %format(temp2['FaceMatches'][0]['Similarity'])
         print "                                           "
         print "********************************************************************************"
         ...
         if abs(diff) > 5:   #abs takes care of negative values and provides a positive number as the result 
          print "***********************************************"
          print "                                               "
          print "  !!! TW: Speech To Text happens here !!!!     "
          print "                                               "
          print "***********************************************"

          #Write datetime.datetime.now to a log file for comparison on the next iteration
          ...
          #Performing Speech To Text actions
          subprocess.call('espeak \" Hi Trevor Welcome back \" ', shell=True)
          WeatherProcessing()
         else:
          print "****************************************************************************"
          print "                                                                            "
          print ("TW: Ain't bothering you because we just spotted you less than a min ago    ")
          print "                                                                            "
          print "****************************************************************************"
        ...
        for nomatch in temp2['UnmatchedFaces']:
         print "***************************************************"
         print "                                                   "
         print " TW:Faces either don't match or are a poor match   "
         print "                                                   "
         print "***************************************************"
	return
...
#########################################################
#AWS Rekognition Code - Face Comparison for Jadyn W
#########################################################
def compare_faces_jw(bucket, key, bucket_target, key_target, threshold=80, region="us-west-2"):

        def WeatherProcessing():
         #OWM Weather Data Functions
         owm = pyowm.OWM('xxxxxx')  # You MUST provide a valid API key
        ...
        print "**********************************************************************************"
        print "                                                                                  "
        print "JW: Source Face Confidence in %s " %format(temp2['SourceImageFace']['Confidence'])
        print "                                                                                  "
        print "**********************************************************************************"
        for match in temp2['FaceMatches']:
         print "*******************************************************************************************"
         print "                                                                                           "
         print "JW: Similarity between compared faces is %s " %format(temp2['FaceMatches'][0]['Similarity'])
         print "                                                                                           "
         print "*******************************************************************************************"
         #Getting the timestamp
         now = datetime.datetime.now()
         ...
         #Reading older timestamp from log file
         proc = subprocess.Popen(["tail -n 1 /opt/data/jw_face_capture_time_log.txt  | cut -d : -f 2"], stdout=subprocess.PIPE, shell=True)
         (out, err) = proc.communicate()
         temp = out.strip()
         oldtimestampminute = int(temp)
         ...
         if abs(diff) > 5:   #abs takes care of negative values and provides a positive number as the result
          print "************************************************"
          print "                                                "
          print "   !!! JW : Speech To Text happens here !!!!    "
          print "                                                "
          print "************************************************"

          #Write datetime.datetime.now to a log file for comparison on the next iteration
          outputFile = open('/opt/data/jw_face_capture_time_log.txt', 'a')
          outputWriter = csv.writer(outputFile)
          tempArray = [now]
          outputWriter.writerow(tempArray)
          outputFile.close()
          ...
          #Performing Speech To Text actions
          subprocess.call('espeak \" Miss Jades its good to see you again \" ', shell=True)
          WeatherProcessing()
         else:
          print "*****************************************************************************"
          print "                                                                             "
          print ("JW: Ain't bothering you because we just spotted you less than a min ago     ")
          print "                                                                             "
          print "*****************************************************************************"
        ...
        for nomatch in temp2['UnmatchedFaces']:
         print "***************************************************"
         print "                                                   "
         print "JW: Faces either don't match or are a poor match   "
         print "                                                   "
         print "***************************************************"
        return
...
#########################################################
#AWS Rekognition Code - Face Comparison for Viveka W
#########################################################
def compare_faces_vw(bucket, key, bucket_target, key_target, threshold=80, region="us-west-2"):

        def WeatherProcessing():
         #OWM Weather Data Functions
         owm = pyowm.OWM('xxxxxxxxx')  # You MUST provide a valid API key
        ...
        print "***********************************************************************************"
        print "                                                                                   "
        print "VW: Source Face Confidence in %s " %format(temp2['SourceImageFace']['Confidence'])
        print "                                                                                   "
        print "***********************************************************************************"
        for match in temp2['FaceMatches']:
         print "***********************************************************************************"
         print "                                                                                   "
         print "VW: Similarity between compared faces is %s " %format(temp2['FaceMatches'][0]['Similarity'])
         print "                                                                                   "
         print "***********************************************************************************"
         #Getting the timestamp
         now = datetime.datetime.now()
         ...
         #Reading older timestamp from log file
         proc = subprocess.Popen(["tail -n 1 /opt/data/vw_face_capture_time_log.txt  | cut -d : -f 2"], stdout=subprocess.PIPE, shell=True)
         (out, err) = proc.communicate()
         temp = out.strip()
         oldtimestampminute = int(temp)
         ...
         if abs(diff) > 5:   #abs takes care of negative values and provides a positive number as the result
          print "**************************************************"
          print "                                                  "
          print "   !!! VW: Speech To Text happens here!!!!        "
          print "                                                  "
          print "**************************************************"

          #Write datetime.datetime.now to a log file for comparison on the next iteration
          outputFile = open('/opt/data/vw_face_capture_time_log.txt', 'a')
          outputWriter = csv.writer(outputFile)
          tempArray = [now]
          outputWriter.writerow(tempArray)
          outputFile.close()
          ...
          #Performing Speech To Text actions
          subprocess.call('espeak \" Hello Madam Viveka welcome back \" ', shell=True)
          WeatherProcessing()
         else:
          print "****************************************************************************"
          print "                                                                            "
          print ("VW: Ain't bothering you because we just spotted you less than a min ago")
          print "                                                                            "
          print "****************************************************************************"
        ...
        for nomatch in temp2['UnmatchedFaces']:
         print "***************************************************"
         print "                                                   "
         print "VW: Faces either don't match or are a poor match"
         print "                                                   "
         print "***************************************************"
        return
...
while True:
 ...
 if len(faces) > 0:
  ...
  ##################################################################
  #Comparing captured image to Trevor's face using AWS Rekognition
  ##################################################################
  bucket_target_var = "tw37-opencv"
  ...
  #Forking a thread to perform the AWS Rekognition Comparison for Trevor
  threads = []
  ...
  ##################################################################
  #Comparing captured image to Jadyn's face using AWS Rekognition
  ##################################################################
  bucket_target_var = "tw37-opencv"
  #key_target_var = "new_image_name.jpg"
  key_source_var = "jadyn_original_1307.jpg"
  bucket_source_var = "tw37-original"  

  #Forking a thread to perform the AWS Rekognition Comparison for Trevor
  threads = []
  t = threading.Thread(target=compare_faces_jw, args=(bucket_source_var, key_source_var, bucket_target_var, imagename))
  threads.append(t)
  t.start() 

  ##################################################################
  #Comparing captured image to Viveka's face using AWS Rekognition
  ##################################################################
  bucket_target_var = "tw37-opencv"
  #key_target_var = "new_image_name.jpg"
  key_source_var = "viveka_original_1307.jpg"
  bucket_source_var = "tw37-original"

  #Temporarily disabling identification for veki
  #Forking a thread to perform the AWS Rekognition Comparison for Trevor
  #threads = []
  #t = threading.Thread(target=compare_faces_vw, args=(bucket_source_var, key_source_var, bucket_target_var, imagename))
  #threads.append(t)
  #t.start()
